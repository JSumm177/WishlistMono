...
export function getPlatformSearchUrls(
  year: number,
  make: string,
  model: string,
  trim?: string | null,
...
function estimateBaselinePrice(
  year: number,
  make: string,
  model: string,
  trim?: string | null,
  ...
  if (
    ...
    combinedText.includes("gt2") ||
    combinedText.includes("gt4") ||
    combinedText.includes(" gt3") ||
    combinedText.includes(" gt2")
  ) {
    trimMultiplier = 2.2;
  } else if (
    combinedText.includes("black series") ||
    combinedText.includes("superleggera")
  ) {
    trimMultiplier = 2.0;
  } else if (combinedText.includes("turbo s")) {
    trimMultiplier = 1.8;
  } else if (
    combinedText.includes("turbo") ||
    combinedText.includes("zr1") ||
    combinedText.includes("hellcat") ||
    combinedText.includes("demon") ||
    combinedText.includes("plaid")
  ) {
    trimMultiplier = 1.65;
  } else if (
    combinedText.includes("z06") ||
    combinedText.includes("gt500") ||
    combinedText.includes("raptor") ||
    combinedText.includes(" trx ")
  ) {
    trimMultiplier = 1.5;
  } else if (
    ...
    combinedText.includes("type r") ||
    combinedText.includes("type s") ||
    combinedText.includes("quadrifoglio") ||
    combinedText.includes("rs ")
  ) {
    trimMultiplier = 1.35;
  } else if (
    combinedText.includes("denali") ||
    combinedText.includes("platinum") ||
    combinedText.includes("limited") ||
  ...
  const depreciationRate = isPremium ? 0.94 : 0.9;
...
export async function scrapeMarketPrices(
  ...
  const baseline =
    wishlistPriceCents || estimateBaselinePrice(year, make, model, trim);
  ...
  const fetchBringATrailer = async (): Promise<number> => {
    ...
    if (html) {
      ...
      if (prices.length > 0) {
        ...
        console.log(
          `[Scraper] Live Bring a Trailer Scrape Success: $${(avg / 100).toLocaleString()}`,
        );
        return avg;
      }
    }
  ...
  const fetchCarsAndBids = async (): Promise<number> => {
    ...
    if (html) {
      ...
      if (prices.length > 0) {
        ...
        console.log(
          `[Scraper] Live Cars & Bids Scrape Success: $${(avg / 100).toLocaleString()}`,
        );
        return avg;
      }
    }
