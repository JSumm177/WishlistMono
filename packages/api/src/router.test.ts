...
describe("appRouter", () => {
  ...
  describe("getVehicles", () => {
    it("should return vehicles with correctly formatted prices, handling nulls and calculating averages", async () => {
      ...
      const mockPrices = [
        { id: 10, vehicleId: 1, source: "cargurus", price: 12200000, url: "http://cargurus" },
        { id: 11, vehicleId: 1, source: "carmax", price: 12400000, url: "http://carmax" },
      ];

      const mockWhereVehicles = jest.fn().mockResolvedValue(mockVehicles);
      const mockFromVehicles = jest.fn().mockReturnValue({ where: mockWhereVehicles });

      const mockWherePrices = jest.fn().mockResolvedValue(mockPrices);
      const mockFromPrices = jest.fn().mockReturnValue({ where: mockWherePrices });
      ...
      expect(result).toEqual([
        { id: 1, make: "Porsche", model: "911", price: 120000, averageMarketPrice: 123000 },
        { id: 2, make: "Toyota", model: "AE86", price: null, averageMarketPrice: null },
      ]);
  ...
  describe("deleteVehicle", () => {
    it("should delete a vehicle and its image when authorized", async () => {
      ...
      const mockVehicle = { id: 1, userId: mockUserId, imageUrl: "https://utfs.io/f/test-key" };
    ...
    it("should throw error if vehicle not found", async () => {
      ...
      await expect(caller.deleteVehicle({ id: 999 })).rejects.toThrow("Vehicle not found");
    });
  ...
  describe("getMarketPrices", () => {
    it("should return market prices for a vehicle", async () => {
      const mockVehicle = { id: 1, userId: mockUserId, make: "Porsche", model: "911", year: 2020 };
      const mockSelectVehicleLimit = jest.fn().mockResolvedValue([mockVehicle]);
      const mockSelectVehicleWhere = jest.fn().mockReturnValue({ limit: mockSelectVehicleLimit });
      const mockSelectVehicleFrom = jest.fn().mockReturnValue({ where: mockSelectVehicleWhere });

      const mockPrices = [
        { id: 10, vehicleId: 1, source: "cargurus", price: 12200000, url: "http://cargurus" },
      ];
      const mockSelectPricesWhere = jest.fn().mockResolvedValue(mockPrices);
      const mockSelectPricesFrom = jest.fn().mockReturnValue({ where: mockSelectPricesWhere });
      ...
      expect(result).toEqual([
        { id: 10, vehicleId: 1, source: "cargurus", price: 122000, url: "http://cargurus" },
      ]);
    });
  });
  ...
  describe("refreshMarketPrices", () => {
    it("should clear old and insert new market prices", async () => {
      const mockVehicle = { id: 1, userId: mockUserId, make: "Porsche", model: "911", year: 2020, price: 12000000 };
      const mockSelectLimit = jest.fn().mockResolvedValue([mockVehicle]);
      const mockSelectWhere = jest.fn().mockReturnValue({ limit: mockSelectLimit });
      const mockSelectFrom = jest.fn().mockReturnValue({ where: mockSelectWhere });
      (db.select as jest.Mock).mockReturnValue({ from: mockSelectFrom });
      ...
      const mockInserted = [
        { id: 10, vehicleId: 1, source: "cargurus", price: 3000000, url: "http://cargurus.test" },
      ];
      const mockInsertValues = jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue(mockInserted) });
      ...
      expect(result).toEqual([
        { id: 10, vehicleId: 1, source: "cargurus", price: 30000, url: "http://cargurus.test" },
      ]);
    });
  });
});
